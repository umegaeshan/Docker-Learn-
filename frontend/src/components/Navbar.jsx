    ...
    <nav className="sticky top-0 z-50 bg-gray-900/90 backdrop-blur-md border-b border-gray-800 shadow-lg">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          ...
          {/* ========================================== */}
          {/* 1. Logo (Desktop සහ Mobile දෙකටම පොදුයි) */}
          {/* ========================================== */}
          ...
          {/* ========================================== */}
          {/* 2. DESKTOP VIEW (මෙය කිසිසේත්ම වෙනස් කර නැත) */}
          {/* ========================================== */}
          ...
          {/* ========================================== */}
          {/* 3. MOBILE VIEW (ජංගම දුරකථනයෙන් බලන විට පමණක් වෙනස් වන කොටස) */}
          {/* ========================================== */}
          <div className="flex md:hidden items-center space-x-2 sm:space-x-4">
            
            {/* 3.1 Mobile Cart Icon */}
            <Link to="/cart" className="p-2 bg-gray-800/50 border border-gray-700/50 rounded-full text-gray-300 hover:text-white">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.8} stroke="currentColor" className="w-6 h-6">
                <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 0 0-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 0 0-16.536-1.84M7.5 14.25 5.106 5.272M6 20.25a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Zm12.75 0a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Z" />
              </svg>
            </Link>

            {/* 3.2 Mobile Profile Icon & Logout Dropdown */}
            {userInfo ? (
              <div className="relative">
                <button onClick={() => setIsProfileOpen(!isProfileOpen)} className="p-2 bg-gray-800/50 border border-gray-700/50 rounded-full text-gray-300 hover:text-white focus:outline-none">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.8} stroke="currentColor" className="w-6 h-6">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A17.933 17.933 0 0 1 12 21.75c-2.676 0-5.216-.584-7.499-1.632Z" />
                  </svg>
                </button>
                ...
                {isProfileOpen && (
                  <div className="absolute right-0 mt-3 w-48 bg-white rounded-xl shadow-2xl py-2 z-50 border border-gray-100">
                    <div className="px-4 py-2 text-sm text-gray-900 border-b border-gray-100 font-bold truncate">Hi, {userInfo.name}</div>
                    <div className="p-1">
                      <button onClick={logoutHandler} className="w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-red-50 rounded-lg font-bold">Logout</button>
                    </div>
                  </div>
                )}
              </div>
      ...
      {/* ========================================== */}
      {/* 4. MOBILE DROPDOWN MENU LINKS (Menu එක එබූ විට එන කොටස) */}
      {/* ========================================== */}
      <div className={`md:hidden overflow-hidden transition-all duration-300 ease-in-out ${isMenuOpen ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'}`}>
        <div className="px-4 pt-2 pb-6 space-y-2 bg-gray-900/95 backdrop-blur-md border-t border-gray-800 shadow-inner">
          ...
          <Link to="/about" onClick={() => setIsMenuOpen(false)} className="block px-4 py-3 rounded-xl text-lg font-medium text-gray-300 hover:bg-gray-800 hover:text-white transition-colors">About Us</Link>
